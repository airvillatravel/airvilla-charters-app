...
export default function DateFilter<
  T extends MasterUserDataType | TeamMemberDataType
>({
  filterFormData,
  setFilterFormData,
  labelName,
  ...
  filterFormData: T;
  setFilterFormData: React.Dispatch<React.SetStateAction<T>>;
  labelName?: string;
  fieldName: keyof T;
}) {
  const options = [
  ...
  useEffect(() => {
    ...
    const selectedElm = options.find(
      (elm) =>
        elm.period.toLowerCase().trim() === String(value)?.toLowerCase().trim()
    );
                    ...
                    {({ active }) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setFilterFormData({
                            ...filterFormData,
                            [fieldName]: option.period.toLowerCase(),
                          });
                        }}
                      >
                        <Check
